      ...
      <svg viewBox={`0 0 ${W} ${H}`} className="w-full h-auto" style={{ minWidth: 320 }}>
        {yTicks.map((v, i) => (
          <g key={i}>
            ...
            <text x={PAD.l - 6} y={py(v) + 4} textAnchor="end" fontSize={9} fill="#52525b">{fmtK(v)}</text>
          </g>
        ))}
...
export default function BacktestSimPanel() {
  ...
  const run = async () => {
    ...
    try {
      const res = await fetch('/api/backtest-sim', {
        method: 'POST',
  ...
  return (
    <div className="pb-8">
      ...
      {/* ── 헤더 ── */}
      <div className="mb-6">
        <h2 className="text-lg font-bold text-zinc-100 mb-1">포트폴리오 시뮬레이터</h2>
        <p className="text-xs text-zinc-500">
          실제 자본을 투입해 매매했을 때의 성과를 시뮬레이션합니다.
          손절·트레일링 스탑·포지션 사이징이 모두 반영됩니다.
        </p>
      </div>
      ...
      {result && m && (
        <div ref={resultRef}>
          ...
          {/* 요약 메트릭 */}
          <div className="mb-6">
            <h3 className="text-sm font-semibold text-zinc-400 uppercase tracking-widest mb-3">성과 요약</h3>
            <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-5 gap-3">
              ...
              />
              <MetricCard
                label="연환산 (CAGR)" value={sign(m.cagr)}
              ...
              />
              <MetricCard
                label="손익비 (PF)" value={m.profitFactor >= 999 ? '∞' : fmt(m.profitFactor)}
                sub="총수익 / 총손실"
          ...
          <div className="mb-6 p-4 rounded-xl border border-zinc-800 bg-zinc-900/50">
            <h3 className="text-[10px] text-zinc-500 uppercase tracking-widest mb-2">전략 평가</h3>
          ...
          {result.equityCurve.length > 1 && (
            <div className="mb-6 p-4 rounded-xl border border-zinc-800 bg-zinc-900/50">
              <h3 className="text-[10px] text-zinc-500 uppercase tracking-widest mb-3">자산 추이</h3>
          ...
          {/* 종목별 성과 */}
          <div className="mb-6">
            <h3 className="text-sm font-semibold text-zinc-400 uppercase tracking-widest mb-3">종목별 성과</h3>
                ...
                <div key={tr.ticker} className="bg-zinc-900 border border-zinc-800 rounded-xl p-4">
                  ...
                  {tr.trades > 0 && (
                    <div className="mt-2 h-1 bg-zinc-800 rounded-full overflow-hidden">
                      <div className="h-full bg-emerald-600 rounded-full" style={{ width: `${tr.winRate}%` }} />
                    </div>
                  )}
                  {tr.trades === 0 && (
          ...
          <div className="p-4 rounded-xl border border-zinc-800 bg-zinc-900/30 text-[10px] text-zinc-600 leading-relaxed">
            ...
            <p>• 당일 신호 → 익일 시가 진입으로 Look-ahead bias 최소화</p>
            <p>• VCP 패턴은 백테스트에서 제외됨 (주봉 롤링 계산 한계)</p>
            <p>• 과거 성과가 미래 수익을 보장하지 않음. 소액 실전 검증 후 확대 운용할 것</p>
          </div>
        </div>
      )}
    </div>
