...
export default function WatchlistManager({ watchlist, onAdd, onRemove, maxTickers = 50 }: Props) {
  ...
  return (
    <div className="mb-6">
      <div className="text-[10px] text-zinc-600 uppercase tracking-widest mb-3">
        관심 종목 ({watchlist.length}/{maxTickers})
      </div>
      <div className="flex flex-wrap gap-2 mb-3">
        {watchlist.map(t => (
          <div key={t} className="flex items-center gap-1.5 px-3 py-1.5 bg-zinc-900 border border-zinc-700 rounded-lg text-sm font-medium text-zinc-200">
            {t}
            <button onClick={() => onRemove(t)} className="text-zinc-600 hover:text-red-400 transition-colors text-xs leading-none ml-0.5">✕</button>
          </div>
        ))}
      </div>
