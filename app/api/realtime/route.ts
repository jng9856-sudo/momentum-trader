...
async function getYahooPriceForTicker(
  ticker: string
...
export async function GET(req: NextRequest) {
  ...
  if (tickers.length === 1) {
    ...
    return NextResponse.json({
      ...
      ...data,
      isRealtime:    false,
      marketSession: 'CLOSED',   // Yahoo는 시간외 데이터 없음을 명시
      extPrice:      null,
      extChangePct:  null,
    });
  }
  ...
  const results = await Promise.all(
    tickers.map(async t => {                               // ← 버그 수정
      ...
      return d
        ? { ticker: t, ...d, isRealtime: false, marketSession: 'CLOSED', extPrice: null, extChangePct: null }
        : null;
    })
  );
